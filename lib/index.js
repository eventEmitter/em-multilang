	...
	module.exports = new Class( {
		...
		, request: function( request, response, next ){
			...
			if (!isHtml || (request.method !== 'get' && request.method !== 'head')) return next();
			...
			// set the language cookie for later visits or visits wihtout language in the url
			if (changed) response.setHeader('set-cookie', 'lang='+request.language+';Path=/;Expires='+new Date(2100, 1, 1).toGMTString());
